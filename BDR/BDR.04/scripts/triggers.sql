...
-- 1. Cria a tabela para registrar as operações de inserção.
...
-- 2. Adiciona a coluna para rastrear a data/hora da última medição no reservatório.
...
-- --------------------------------------------------------------------
-- TRIGGER 2: VALIDAR DADOS DE ENTRADA (FAIXAS ACEITÁVEIS)
-- Objetivo: Impedir a inserção ou atualização de valores de pH e
-- Oxigênio Dissolvido (_do) fora das faixas aceitáveis.
...
BEGIN
  ...
  IF NEW.ph IS NOT NULL AND (NEW.ph < 0.0 OR NEW.ph > 14.0) THEN
    RAISE EXCEPTION 'ERRO DE VALIDAÇÃO: O valor do pH (%) deve estar entre 0.0 e 14.0.', NEW.ph;
  END IF;
  ...
  IF NEW._do IS NOT NULL AND (NEW._do < 0.0 OR NEW._do > 20.0) THEN
    RAISE EXCEPTION 'ERRO DE VALIDAÇÃO: O valor do Oxigênio Dissolvido (%) deve ser positivo e não exceder 20.0 mg/L.', NEW._do;
  END IF;
...
-- --------------------------------------------------------------------
-- TRIGGER 3: ATUALIZAÇÃO AUTOMÁTICA DE ENTIDADE RELACIONADA
-- Objetivo: Atualizar o campo 'ultima_atualizacao' na tabela
-- 'tbreservatorio' sempre que um novo dado for inserido em um sítio
-- pertencente a ele.
-- Evento: AFTER INSERT na tbmedidacamposuperficie.
-- --------------------------------------------------------------------
...
BEGIN
  -- Encontrar o ID do reservatório a partir do ID do sítio da nova linha
