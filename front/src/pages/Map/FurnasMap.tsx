...
// --- Componente: SidebarItemFurnas (REFATORADO) ---
...
// --- Componente: SidebarFurnas (REFATORADO) ---
...
// Ícone customizado 'siteIconFurnas' REMOVIDO para simplificar
...
const FurnasMap: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const transformedSitios: Sitio[] = (sitiosData.data || []).map(
          (apiSitio) => ({
            nome: apiSitio.sitio,
            lat: apiSitio.reservatorio_lat,
            lng: apiSitio.reservatorio_lng,
            idreservatorio: apiSitio.idreservatorio,
            descricao: apiSitio.descricao,
          }),
        );

        setSitios(transformedSitios);
      } catch (error) {
  ...
  // Variavel popupStyles REMOVIDA

  if (loading) {
    return <div style={{ padding: "1rem" }}>Carregando mapa FURNAS...</div>;
  }
  ...
  return (
    <div className="map-layout-container">
      ...
      <div className="map-main-area">
        <h1 className="map-main-title">Mapa de Localizações - Projeto FURNAS</h1>
        <MapContainer
          ...
          {filteredMapSitios.map((sitio) => {
            ...
            return sitio.lat && sitio.lng ? (
              <Marker
                ...
                // Prop 'icon' REMOVIDA (usa o padrão)
              >
                <Popup className="custom-map-popup">
